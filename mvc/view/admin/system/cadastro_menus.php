...
<div id="app">
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="sm-12">
                <form class="cadastro" id="cadastro_menu" method="POST" onsubmit="return false;">
                    <div class="form-row align-items-center">
                        ...
                        <div class="input-group mb-3">
                            <label class="sr-only" for="inlineFormInputGroupmenu">Menu Pai</label>
                            <div class="input-group-prepend">
                                <div class="input-group-text h-100">
...
<script>
    var app = new Vue({
        ...
        data: {
            ...
            infoMsg: "",
            state: 'default',
            serverUrl: '/server/menus',
            elements: [],
            parentMenus: [],
            elementCurrent: { id: "", id_menu: "", nome: "", ocultar: false },
            pagination: {
                page: 1,
                rowCount: 10,
                total: 0,
        ...
        methods: {
            ...
            prepareNew() {
                this.clearMsg();
                this.elementCurrent = { id: "", id_menu: "", nome: "", ocultar: false };
                this.state = 'new';
            },
            cancelAction() {
                this.errorMsg= "";
            ...
            getMenuName(id) {
                if(!id) return "";
                const menu = this.parentMenus.find(m => m.id == id);
                return menu ? menu.nome : id;
            },
            loadParentMenus() {
                // Fetch all menus to populate the parent menu dropdown
                // We assume we can get all menus without pagination for the dropdown, or at least a large number
                // For now, we'll try fetching with a large rowCount or just the default findall
                let params = new URLSearchParams();
                params.append('row_count', 1000); // Fetch up to 1000 menus for dropdown
                params.append('token', this.generateToken(256));
                
                axios.get(this.serverUrl+"?"+params.toString()).then(response => {
                   if(response.data.elements) {
                       this.parentMenus = response.data.elements;
                   }
                }).catch(error => {
                ...
                if(this.elementCurrent?.id){
                    axios.put(this.serverUrl+"/"+this.elementCurrent.id, formData).then(response => {
                        ...
                        if (this.successMsg) {
                            ...
                            this.loadParentMenus(); // Refresh parent menus incase name changed
                        }
                    }).catch((error) => {
                        console.log(error);
					...
					axios.post(this.serverUrl, formData).then(response => {
						...
						if (this.successMsg) {
							...
							this.elementCurrent = {id:"", id_menu: "", nome: "", ocultar: false};
							this.findAllElements(this.pagination.page);
                            this.loadParentMenus(); // Refresh parent menus
						}
					}).catch((error) => {
						console.log(error);
						this.errorMsg = "Erro desconhecido: " + error;
