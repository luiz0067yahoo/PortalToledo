...
if ($isApiRequest) {
    ...
    require_once($_SERVER['DOCUMENT_ROOT'].'/mvc/controller/controllerVideos.php');
        ...
        <div class="col-sm-12">
            <form @submit.prevent="enviarCadastro">
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="text" class="form-control" v-model="form.nome" placeholder="Nome do Vídeo">
                </div>

                <div class="input-group mb-3">
                    <span class="input-group-text"><i class="fa fa-video-camera"></i></span>
                    <input type="text" class="form-control" v-model="form.video" placeholder="Link/Nome do Vídeo">
                </div>
...
createApp({
    setup() {
        ...
        async function carregarAlbums() {
             try {
                // Assuming album videos also uses this hybrid structure or existing server logic.
                // Existing file used '/server/albumVideos'. 
                // We should keep using that or if that file is also refactored, use its path.
                // 'cadastro_album_videos.php' is NOT in the list of files to refactor in this prompt.
                // So we stick to '/server/albumVideos' (which calls controllerAlbumVideos).
                const response = await fetch('/server/albumVideos', {
