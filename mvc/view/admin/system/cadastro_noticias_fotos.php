        ...
        <div class="col-sm-12">
            <form @submit.prevent="enviarCadastro">
                ...
                <div class="input-group mb-3">
                    ...
                    <select v-model="form.id_noticias" class="form-control">
            ...
            <table class="table table-striped mt-4" v-if="lista.length > 0">
                ...
                <tbody>
                    <tr v-for="item in lista" :key="item.id">
                        <td>{{ item.id }}</td>
                        <td>{{ getNoticiaTitle(item.id_noticias) }}</td>
                        <td>{{ item.nome }}</td>
                        <td>
                            <a v-if="item.foto" :href="'/uploads/noticias_fotos/' + item.foto" target="_blank" class="btn btn-sm btn-info">Ver</a>
                        </td>
                        <td>
...
createApp({
    setup() {
        ...
        function getNoticiaTitle(id) {
            const n = noticias.value.find(x => x.id == id);
        ...
        async function buscarPorId() {
            if(!form.value.id) return;
             try {
                // For hybrid, we pass ID via GET if simplest, or could use POST findById
