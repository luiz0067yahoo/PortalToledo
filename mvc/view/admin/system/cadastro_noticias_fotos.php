...
<div id="app" class="container">
    ...
    <h1>CADASTRO DE FOTOS NOTÍCIA <?php echo $idNoticia; ?></h1>
    <br>
            ...
            <form class="cadastro" id="cadastro_foto" method="POST" enctype="multipart/form-data" onsubmit="return false;">
                ...
                <div class="row">
                    <div class="col-md-12 d-flex gap-2">
                        ...
                        <a target="_blank" :href="'/admin/noticiasFotos/' + elementCurrent.id"><button v-if="state=='edit'" type="button" class="btn btn-primary "><i class="fa fa-camera"></i> Cadastrar Fotos</button></a>
                ...
                <tbody>
                    <tr v-for="element in elements" :key="element.id">
                        ...
                        <td>
                            <img 
                                v-if="element.foto"
                                :src="`/uploads/noticias_fotos/original/${element.foto}`"
                                @error.once="($event) => {
                                    $event.target.src = `/uploads/noticias/original/${element.foto}`
                                    //$event.target.style.display = 'none';
                                }"
                                style="height:60px; width:auto; cursor:pointer; border-radius:4px;"
                                @click="openImageModal(`/uploads/noticias_fotos/1024x768/${element.foto}`)"
                            >
                        </td>
                        <td>{{ element.nome }}</td>
                        <td>
...
createApp({
    setup() {
        ...
        // Config
        const serverUrl = '/server/noticiasFotos';
        ...
        const imagemExiste = (url) => {
            return new Promise((resolve) => {
                const img = new Image();
                img.onload = () => resolve(true);
                img.onerror = () => resolve(false);  // ou reject, mas true/false é mais prático aqui
                img.src = url;

                // Caso a imagem já esteja em cache e carregada instantaneamente
                if (img.complete) {
                    resolve(true);
                }
            });
        ...
        // Exemplo de uso (não recomendado no v-for por performance)
        const fotoCorreta = async (foto) => {
            if (!foto) return null;

            const caminho1 = `/uploads/noticias_fotos/original/${foto}`;
            const caminho2 = `/uploads/noticias/original/${foto}`;

            if (await imagemExiste(caminho1)) return caminho1;
            if (await imagemExiste(caminho2)) return caminho2;
        ...
        const getIdFromUrl = (url) => {
            try {
                ...
                // Caso a URL seja inválida, tenta método mais simples
        ...
        const imagemPreviewOuAtual = computed(() => {
            ...
            if (elementCurrent.value.foto) {
                return `/uploads/noticias_fotos/original/${elementCurrent.value.foto}`;
            }
            return '';
        });
        ...
        const prepareNew = () => {
            clearMsg();
            elementCurrent.value = { id: '', id_noticia: id_noticia_route, nome: '', ocultar: false, foto: '' };
            files.foto.value = null;
            fotoPreview.value = '';
            foto_base64.value = '';
            fotos_struct.value = [];
