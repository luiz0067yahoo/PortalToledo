...
<div id="app" class="container">
    ...
    <h1>CADASTRO DE FOTOS ANÚNCIO <?php echo $idAnuncio; ?></h1>
    <br>
            ...
            <table class="table table-striped mt-4" v-if="elements.length > 0">
                ...
                <tbody>
                    <tr v-for="element in elements" :key="element.id">
                        ...
                        <td>
                            <img v-if="element.foto" :src="`/uploads/anuncio/original/${element.foto}`"
                                 style="height:60px; width:auto; cursor:pointer; border-radius:4px;"
                                 @click="openImageModal(`/uploads/anuncio/1024x768/${element.foto}`)">
                        </td>
                        <td>{{ element.nome }}</td>
                        <td>
...
createApp({
    setup() {
        ...
        // Config
        const serverUrl = '/server/anunciosFotos';
        ...
        const imagemPreviewOuAtual = computed(() => {
            ...
            if (elementCurrent.value.foto) {
                return `/uploads/anuncio/original/${elementCurrent.value.foto}`;
            }
            return '';
        });
        ...
        const prepareNew = () => {
            clearMsg();
            elementCurrent.value = { id: '', id_anuncio: id_anuncio_route, nome: '', ocultar: false, foto: '' };
            files.foto.value = null;
            fotoPreview.value = '';
            foto_base64.value = '';
            fotos_struct.value = [];
